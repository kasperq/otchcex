object FPererasZag: TFPererasZag
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = #1055#1077#1088#1077#1088#1072#1089#1087#1088#1077#1076#1077#1083#1077#1085#1080#1077' '#1079#1072#1075#1088#1091#1079#1082#1080' '#1087#1086' '#1082#1086#1101#1092#1092#1080#1094#1080#1077#1085#1090#1091
  ClientHeight = 623
  ClientWidth = 1087
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  WindowState = wsMaximized
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Label13: TLabel
    Left = 9
    Top = 83
    Width = 232
    Height = 13
    Margins.Bottom = 0
    Caption = #1055#1088#1077#1087#1072#1088#1072#1090', '#1085#1072' '#1082#1086#1090#1086#1088#1086#1084' '#1086#1089#1090#1072#1074#1083#1103#1090#1100' '#1086#1082#1088#1091#1075#1083#1077#1085#1080#1103':'
  end
  object Label14: TLabel
    Left = 431
    Top = 88
    Width = 275
    Height = 13
    Margins.Bottom = 0
    AutoSize = False
    Caption = 'Label7'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clPurple
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object SpeedButton2: TSpeedButton
    Left = 401
    Top = 84
    Width = 23
    Height = 20
    Hint = #1054#1090#1082#1088#1099#1090#1100' '#1089#1087#1088#1072#1074#1086#1095#1085#1080#1082
    Glyph.Data = {
      
      000000FF00FF0000000000FF00FF00000000FFFFFFFFFF000000}
    ParentShowHint = False
    ShowHint = True
    OnClick = SpeedButton1Click
  end
  object Label15: TLabel
    Left = 712
    Top = 88
    Width = 111
    Height = 13
    Margins.Bottom = 0
    AutoSize = False
    Caption = 'Label10'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clPurple
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object Label16: TLabel
    Left = 834
    Top = 88
    Width = 164
    Height = 13
    Margins.Bottom = 0
    AutoSize = False
    Caption = 'Label11'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clPurple
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ParentFont = False
  end
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 1087
    Height = 33
    ButtonHeight = 30
    ButtonWidth = 32
    Caption = 'ToolBar1'
    Images = ImageList2
    ParentShowHint = False
    ShowHint = True
    TabOrder = 0
    object ToolButton4: TToolButton
      Left = 0
      Top = 0
      Hint = #1059#1076#1072#1083#1080#1090#1100' '#1088#1072#1089#1087#1088#1077#1076#1077#1083#1077#1085#1080#1077
      Caption = 'ToolButton4'
      ImageIndex = 1
      OnClick = ToolButton4Click
    end
    object ToolButton1: TToolButton
      Left = 32
      Top = 0
      Hint = #1042#1099#1073#1088#1072#1090#1100' '#1087#1088#1077#1087#1072#1088#1072#1090#1099' '#1076#1083#1103' '#1088#1072#1089#1087#1088#1077#1076#1077#1083#1077#1085#1080#1103
      Caption = 'ToolButton1'
      ImageIndex = 6
      OnClick = ToolButton1Click
    end
    object ToolButton2: TToolButton
      Left = 64
      Top = 0
      Hint = #1056#1072#1089#1095#1077#1090
      Caption = 'ToolButton2'
      ImageIndex = 7
      OnClick = ToolButton2Click
    end
    object ToolButton3: TToolButton
      Left = 96
      Top = 0
      Cursor = crHandPoint
      Hint = #1057#1086#1093#1088#1072#1085#1080#1090#1100' '#1080#1079#1084#1077#1085#1077#1085#1085#1080#1103' '
      Caption = 'ToolButton3'
      ImageIndex = 2
      OnClick = ToolButton3Click
    end
    object ToolButton5: TToolButton
      Left = 128
      Top = 0
      Cursor = crHandPoint
      Hint = #1055#1077#1095#1072#1090#1100' '
      Caption = 'ToolButton5'
      ImageIndex = 4
      OnClick = ToolButton5Click
    end
    object Label5: TLabel
      Left = 160
      Top = 0
      Width = 284
      Height = 30
      Margins.Bottom = 0
      Alignment = taRightJustify
      AutoSize = False
      BiDiMode = bdLeftToRight
      Caption = #1047#1072#1076#1072#1081#1090#1077' '#1084#1077#1089#1103#1094' '#1080' '#1075#1086#1076' '#1076#1083#1103' '#1087#1077#1088#1077#1088#1072#1089#1087#1088#1077#1076#1077#1083#1077#1085#1080#1103' :'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentBiDiMode = False
      ParentFont = False
      Layout = tlCenter
    end
    object SpinEdit3: TSpinEdit
      Left = 444
      Top = 0
      Width = 38
      Height = 30
      Ctl3D = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxValue = 12
      MinValue = 1
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
      Value = 1
      OnChange = SpinEdit3Change
    end
    object SpinEdit4: TSpinEdit
      Left = 482
      Top = 0
      Width = 49
      Height = 30
      Ctl3D = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      MaxValue = 2050
      MinValue = 2000
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 1
      Value = 2000
      OnChange = SpinEdit4Change
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 433
    Width = 1087
    Height = 20
    Align = alTop
    TabOrder = 1
    object Label8: TLabel
      Left = 1
      Top = 2
      Width = 236
      Height = 13
      Margins.Bottom = 0
      Caption = #1055#1077#1088#1077#1088#1072#1089#1087#1088#1077#1076#1077#1083#1077#1085#1085#1072#1103' '#1079#1072#1075#1088#1091#1079#1082#1072' '#1089#1099#1088#1100#1103'  '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 33
    Width = 1087
    Height = 192
    Align = alTop
    TabOrder = 2
    object Label1: TLabel
      Left = 9
      Top = 10
      Width = 259
      Height = 13
      Margins.Bottom = 0
      Caption = #1055#1088#1077#1087#1072#1088#1072#1090', '#1089' '#1082#1086#1090#1086#1088#1086#1075#1086' '#1087#1077#1088#1077#1088#1072#1089#1087#1088#1077#1076#1077#1083#1080#1090#1100' '#1079#1072#1075#1088#1091#1079#1082#1091':'
    end
    object Label2: TLabel
      Left = 9
      Top = 30
      Width = 232
      Height = 13
      Margins.Bottom = 0
      Caption = #1055#1088#1077#1087#1072#1088#1072#1090', '#1085#1072' '#1082#1086#1090#1086#1088#1086#1084' '#1086#1089#1090#1072#1074#1083#1103#1090#1100' '#1086#1082#1088#1091#1075#1083#1077#1085#1080#1103':'
    end
    object Label6: TLabel
      Left = 430
      Top = 12
      Width = 273
      Height = 13
      Margins.Bottom = 0
      AutoSize = False
      Caption = 'Label6'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label7: TLabel
      Left = 430
      Top = 32
      Width = 275
      Height = 13
      Margins.Bottom = 0
      AutoSize = False
      Caption = 'Label7'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object SpeedButton5: TSpeedButton
      Left = 400
      Top = 6
      Width = 24
      Height = 21
      Hint = #1054#1090#1082#1088#1099#1090#1100' '#1089#1087#1088#1072#1074#1086#1095#1085#1080#1082
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        000000FF00FF0000000000FF00FF00000000FFFFFFFFFF000000}
      Layout = blGlyphBottom
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      OnClick = SpeedButton5Click
    end
    object SpeedButton1: TSpeedButton
      Left = 401
      Top = 29
      Width = 23
      Height = 20
      Hint = #1054#1090#1082#1088#1099#1090#1100' '#1089#1087#1088#1072#1074#1086#1095#1085#1080#1082
      Glyph.Data = {
        
        000000FF00FF0000000000FF00FF00000000FFFFFFFFFF000000}
      ParentShowHint = False
      ShowHint = True
      OnClick = SpeedButton1Click
    end
    object Label4: TLabel
      Left = 714
      Top = 12
      Width = 112
      Height = 13
      Margins.Bottom = 0
      AutoSize = False
      Caption = 'Label4'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label9: TLabel
      Left = 832
      Top = 12
      Width = 164
      Height = 13
      Margins.Bottom = 0
      AutoSize = False
      Caption = 'Label9'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label10: TLabel
      Left = 714
      Top = 32
      Width = 111
      Height = 13
      Margins.Bottom = 0
      AutoSize = False
      Caption = 'Label10'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label11: TLabel
      Left = 832
      Top = 32
      Width = 164
      Height = 13
      Margins.Bottom = 0
      AutoSize = False
      Caption = 'Label11'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object SpeedButton3: TSpeedButton
      Left = 401
      Top = 51
      Width = 23
      Height = 19
      Hint = #1054#1090#1082#1088#1099#1090#1100' '#1089#1087#1088#1072#1074#1086#1095#1085#1080#1082
      Glyph.Data = {
        
        000000FF00FF0000000000FF00FF00000000FFFFFFFFFF000000}
      ParentShowHint = False
      ShowHint = True
      OnClick = SpeedButton3Click
    end
    object Label17: TLabel
      Left = 8
      Top = 51
      Width = 247
      Height = 13
      Margins.Bottom = 0
      Caption = #1055#1088#1077#1087#1072#1088#1072#1090', '#1085#1072' '#1082#1086#1090#1086#1088#1086#1084' '#1086#1089#1090#1072#1074#1083#1103#1090#1100' '#1085#1077#1079#1072#1074#1077#1088#1096#1077#1085#1082#1091':'
    end
    object Label18: TLabel
      Left = 430
      Top = 52
      Width = 275
      Height = 13
      Margins.Bottom = 0
      AutoSize = False
      Caption = 'Label18'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label19: TLabel
      Left = 714
      Top = 52
      Width = 111
      Height = 13
      Margins.Bottom = 0
      AutoSize = False
      Caption = 'Label19'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label20: TLabel
      Left = 832
      Top = 52
      Width = 164
      Height = 13
      Margins.Bottom = 0
      AutoSize = False
      Caption = 'Label20'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 8
      Top = 70
      Width = 325
      Height = 13
      Margins.Bottom = 0
      Caption = #1055#1088#1077#1087#1072#1088#1072#1090#1099' '#1084#1077#1078#1076#1091' '#1082#1086#1090#1086#1088#1099#1084#1080'  '#1088#1072#1089#1087#1088#1077#1076#1077#1083#1080#1083#1080' '#1079#1072#1075#1088#1091#1079#1082#1091':'
      Color = clMenuBar
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Edit1: TEdit
      Left = 271
      Top = 6
      Width = 130
      Height = 22
      Color = clWhite
      Ctl3D = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 0
      OnChange = Edit1Change
      OnClick = Edit1Click
      OnKeyDown = Edit1KeyDown
    end
    object Edit2: TEdit
      Left = 271
      Top = 27
      Width = 130
      Height = 22
      Color = clWhite
      Ctl3D = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clPurple
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 1
      OnChange = Edit2Change
      OnClick = Edit2Click
      OnKeyDown = Edit2KeyDown
    end
    object DBGridEh1: TDBGridEh
      Left = 1
      Top = 87
      Width = 1085
      Height = 104
      Align = alBottom
      AutoFitColWidths = True
      DataSource = DSFactVipuskZag
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      FooterColor = 16776176
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clNavy
      FooterFont.Height = -11
      FooterFont.Name = 'MS Sans Serif'
      FooterFont.Style = [fsBold]
      FooterRowCount = 1
      OptionsEh = [dghFixed3D, dghHighlightFocus, dghClearSelection, dghEnterAsTab]
      ParentFont = False
      SumList.Active = True
      TabOrder = 2
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clBlack
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      TitleHeight = 25
      UseMultiTitle = True
      VertScrollBar.Tracking = True
      Columns = <
        item
          EditButtons = <>
          FieldName = 'KOD_PROD'
          Footers = <>
          Title.Caption = #1050#1086#1076' '#1087#1088#1077#1087#1072#1088#1072#1090#1072' '
          Width = 136
        end
        item
          EditButtons = <>
          FieldName = 'NMAT'
          Footers = <>
          Title.Caption = #1053#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077
          Width = 258
        end
        item
          EditButtons = <>
          FieldName = 'REGION'
          Footers = <>
          Title.Caption = #1056#1077#1075#1080#1086#1085
          Width = 183
        end
        item
          EditButtons = <>
          FieldName = 'XARKT'
          Footers = <>
          Title.Caption = #1061#1072#1088#1072#1082#1090#1077#1088#1080#1089#1090#1080#1082#1072
          Width = 151
        end
        item
          EditButtons = <>
          FieldName = 'KOL_PRIH'
          Footer.ValueType = fvtSum
          Footers = <>
          Title.Caption = #1042#1099#1087#1091#1089#1082
          Width = 98
        end
        item
          EditButtons = <>
          FieldName = 'VIPNZ'
          Footers = <>
          Title.Caption = #1055#1083'. '#1074#1099#1087#1091#1089#1082' '#1080#1079' '#1085'/'#1079
          Width = 97
        end>
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 225
    Width = 1087
    Height = 208
    Align = alTop
    TabOrder = 3
    object Label12: TLabel
      Left = 9
      Top = 0
      Width = 406
      Height = 13
      Margins.Bottom = 0
      Caption = #1042#1074#1086#1076' '#1092#1072#1082#1090#1080#1095#1077#1089#1082#1080#1093' '#1086#1089#1090#1072#1090#1082#1086#1074' '#1089#1099#1088#1100#1103' '#1074' '#1085#1077#1079#1072#1074#1077#1088#1096#1077#1085#1085#1086#1084' '#1087#1088#1086#1080#1079#1074#1086#1076#1089#1090#1074#1077
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label21: TLabel
      Left = 538
      Top = 2
      Width = 192
      Height = 13
      Margins.Bottom = 0
      Caption = #1056#1072#1089#1093#1086#1076#1085#1099#1081' '#1082#1086#1077#1092#1092#1080#1094#1080#1077#1085#1090' '#1089#1091#1073#1089#1090#1072#1085#1094#1080#1080':'
    end
    object Label22: TLabel
      Left = 838
      Top = 2
      Width = 133
      Height = 13
      Margins.Bottom = 0
      Caption = #1042#1099#1087#1091#1089#1082' '#1080#1079' '#1085#1077#1079#1072#1074#1077#1088#1096#1077#1085#1082#1080':'
    end
    object DBGridEh2: TDBGridEh
      Left = 1
      Top = 21
      Width = 1085
      Height = 186
      Align = alBottom
      AutoFitColWidths = True
      DataSource = DSFactOstNez
      FooterColor = clWindow
      FooterFont.Charset = DEFAULT_CHARSET
      FooterFont.Color = clWindowText
      FooterFont.Height = -11
      FooterFont.Name = 'MS Sans Serif'
      FooterFont.Style = []
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      UseMultiTitle = True
      Columns = <
        item
          ButtonStyle = cbsNone
          Checkboxes = True
          EditButtons = <>
          FieldName = 'PRSUBST'
          Footers = <>
          KeyList.Strings = (
            '1'
            '0')
          Title.Caption = #1055#1088'. '#1089#1091#1073'.'
          Width = 32
        end
        item
          ButtonStyle = cbsNone
          Checkboxes = True
          EditButtons = <>
          FieldName = 'PR_RASPRED'
          Footers = <>
          KeyList.Strings = (
            '1'
            '0')
          Title.Caption = #1055#1088'. '#1088#1072#1089#1087#1088
          Width = 36
        end
        item
          Alignment = taCenter
          EditButtons = <>
          FieldName = 'KRAZ'
          Footers = <>
          Title.Caption = #1056#1072#1079'.'
          Width = 27
        end
        item
          Alignment = taCenter
          EditButtons = <>
          FieldName = 'KSM_ID'
          Footers = <>
          Title.Caption = #1050#1086#1076
          Width = 60
        end
        item
          EditButtons = <>
          FieldName = 'NMAT'
          Footers = <>
          Title.Caption = #1053#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077' '#1089#1099#1088#1100#1103
          Width = 217
        end
        item
          EditButtons = <>
          FieldName = 'NEIS'
          Footers = <>
          Title.Caption = #1045#1076'.'#1080#1079','
          Width = 47
        end
        item
          EditButtons = <>
          FieldName = 'OSTATOK_BEGIN_S'
          Footers = <>
          Title.Caption = #1054#1073#1097#1080#1081' '#1086#1089#1090'. '#1085#1072' '#1085#1072#1095'. '#1084'-'#1094#1072' |'#1074' '#1089#1099#1088#1100#1077
          Width = 107
        end
        item
          EditButtons = <>
          FieldName = 'OSTATOK_BEGIN_NZ'
          Footers = <>
          Title.Caption = #1054#1073#1097#1080#1081' '#1086#1089#1090'. '#1085#1072' '#1085#1072#1095'. '#1084'-'#1094#1072' |'#1074' '#1085'/'#1079
          Width = 58
        end
        item
          EditButtons = <>
          FieldName = 'PRIX_PERIOD'
          Footers = <>
          Title.Caption = #1055#1088#1080#1093#1086#1076' '#1089#1099#1088#1100#1103' '#1085#1072' '#1087#1088#1077#1087#1072#1088#1072#1090#1099
          Width = 86
        end
        item
          EditButtons = <>
          FieldName = 'ZAGFACT'
          Footers = <>
          Title.Caption = ' '#1047#1072#1075#1088#1091#1079#1082#1072' '
          Width = 70
        end
        item
          EditButtons = <>
          FieldName = 'RASH_VIRAB_PERIOD'
          Footers = <>
          Title.Caption = #1056#1072#1089#1093#1086#1076' '#1085#1072' '#1074#1099#1088#1072#1073#1086#1090#1082#1091
          Width = 78
        end
        item
          EditButtons = <>
          FieldName = 'OSTFACTS'
          Footers = <>
          Title.Caption = #1060#1072#1082#1090'. '#1086#1089#1090'. '#1085#1072' '#1082#1086#1085'. '#1084'-'#1094#1072'| '#1074' '#1089#1099#1088#1100#1077
          Width = 99
        end
        item
          EditButtons = <>
          FieldName = 'OSTFACTNZ'
          Footers = <>
          Title.Caption = #1060#1072#1082#1090'. '#1086#1089#1090'. '#1085#1072' '#1082#1086#1085'. '#1084'-'#1094#1072'| '#1074' '#1085'/'#1079
          Width = 73
        end
        item
          EditButtons = <>
          FieldName = 'KOEF'
          Footers = <>
          Title.Caption = #1050#1086#1101#1092'. '#1088#1072#1089#1087#1088#1077#1076'.  (K)'
          Width = 68
        end
        item
          EditButtons = <>
          FieldName = 'PLNORM'
          Footers = <>
          Title.Caption = #1055#1083#1072#1085'. '#1085#1086#1088#1084#1072'  (N)'
          Width = 57
        end
        item
          EditButtons = <>
          FieldName = 'PROC'
          Footers = <>
          Title.Caption = #1055#1088#1086#1094'.'#1086#1090#1082#1083'. '#1086#1090' '#1085#1086#1088#1084#1099' 100-N/K*100'
          Width = 67
        end>
    end
    object Edit4: TEdit
      Left = 712
      Top = 3
      Width = 90
      Height = 17
      BorderStyle = bsNone
      Color = clBtnFace
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 1
      Text = 'Edit4'
      OnKeyDown = Edit4KeyDown
    end
    object Edit5: TEdit
      Left = 976
      Top = 3
      Width = 81
      Height = 16
      BorderStyle = bsNone
      Color = clBtnFace
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 2
      Text = 'Edit5'
    end
  end
  object Edit3: TEdit
    Left = 271
    Top = 81
    Width = 130
    Height = 22
    Color = clWhite
    Ctl3D = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clPurple
    Font.Height = -15
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentCtl3D = False
    ParentFont = False
    TabOrder = 4
    OnChange = Edit3Change
    OnClick = Edit3Click
    OnKeyDown = Edit3KeyDown
  end
  object DBGridEh3: TDBGridEh
    Left = 0
    Top = 453
    Width = 1087
    Height = 170
    Align = alClient
    AutoFitColWidths = True
    DataSource = DSSyrieZag
    Flat = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    FooterColor = clWindow
    FooterFont.Charset = DEFAULT_CHARSET
    FooterFont.Color = clNavy
    FooterFont.Height = -11
    FooterFont.Name = 'MS Sans Serif'
    FooterFont.Style = [fsBold]
    OptionsEh = [dghFixed3D, dghHighlightFocus, dghClearSelection, dghAutoSortMarking, dghEnterAsTab]
    ParentFont = False
    TabOrder = 5
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clBlack
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    TitleHeight = 25
    UseMultiTitle = True
    VertScrollBar.Tracking = True
    OnSortMarkingChanged = DBGridEh3SortMarkingChanged
    Columns = <
      item
        EditButtons = <>
        FieldName = 'KOD_PROD'
        Footers = <>
        Title.Caption = #1050#1086#1076' '#1087#1088#1077#1087#1072#1088#1072#1090#1072
        Title.SortIndex = 1
        Title.SortMarker = smDownEh
        Width = 145
      end
      item
        EditButtons = <>
        FieldName = 'KRAZ'
        Footers = <>
        Title.Caption = #1056#1072#1079#1076#1077#1083
        Width = 50
      end
      item
        EditButtons = <>
        FieldName = 'KSM_ID'
        Footers = <>
        Title.Caption = #1050#1086#1076
        Width = 68
      end
      item
        EditButtons = <>
        FieldName = 'NMAT'
        Footers = <>
        Title.Caption = #1053#1072#1080#1084#1077#1085#1086#1074#1072#1085#1080#1077' '#1089#1099#1088#1100#1103
        Width = 275
      end
      item
        EditButtons = <>
        FieldName = 'NEIS'
        Footers = <>
        Title.Caption = #1045#1076'.'#1080#1079#1084'.'
        Width = 47
      end
      item
        EditButtons = <>
        FieldName = 'OSTATOK_BEGIN_S'
        Footers = <>
        Title.Caption = #1054#1089#1090#1072#1090#1086#1082' '#1085#1072' '#1085#1072#1095#1072#1083#1086' '#1084#1077#1089#1103#1094#1072' |'#1074' '#1089#1099#1088#1100#1077
      end
      item
        EditButtons = <>
        FieldName = 'OSTATOK_BEGIN_NZ'
        Footers = <>
        Title.Caption = #1054#1089#1090#1072#1090#1086#1082' '#1085#1072' '#1085#1072#1095#1072#1083#1086' '#1084#1077#1089#1103#1094#1072' |'#1074' '#1085#1077#1079#1072#1074#1077#1088#1096#1077#1085#1082#1077
        Width = 101
      end
      item
        DisplayFormat = '########0.000'
        EditButtons = <>
        FieldName = 'PRIX'
        Footers = <>
        Title.Caption = #1055#1088#1080#1093#1086#1076
        Width = 84
      end
      item
        DisplayFormat = '########0.000'
        EditButtons = <>
        FieldName = 'KOL_RASH_EDIZ'
        Footer.Color = clWhite
        Footers = <>
        Title.Caption = #1047#1072#1075#1088#1091#1079#1082#1072
        Width = 92
      end
      item
        DisplayFormat = '########0.000'
        EditButtons = <>
        FieldName = 'RASVIR'
        Footers = <>
        Title.Caption = #1056#1072#1089#1093#1086#1076
        Width = 95
      end>
  end
  object ImageList2: TImageList
    Height = 24
    Width = 24
    Left = 664
    Top = 36
    Bitmap = {
      
      000000000000}
  end
  object DSFactVipuskZag: TDataSource
    DataSet = FactVipuskZag
    Left = 416
    Top = 368
  end
  object DSSyrieZag: TDataSource
    DataSet = SyrieZag
    Left = 432
    Top = 504
  end
  object IBZagruzka: TIBQuery
    Database = DM1.BELMED
    Transaction = DM1.IBT_Read
    SQL.Strings = (
      ''
      ''
      '')
    Left = 576
    Top = 40
  end
  object EIBQuery1: TEIBQuery
    Macros = <>
    EhMacros = <>
    IBDataBase = DM1.BELMED
    IBTransaction = DM1.IBT_Read
    Left = 616
    Top = 40
  end
  object frReport2: TfrReport
    Dataset = FrPererasZag
    InitialZoom = pzDefault
    PreviewButtons = [pbZoom, pbLoad, pbSave, pbPrint, pbFind, pbHelp, pbExit]
    RebuildPrinter = False
    OnGetValue = frReport2GetValue
    Left = 168
    Top = 177
    ReportForm = {19000000}
  end
  object FrPererasZag: TfrDBDataSet
    Left = 216
    Top = 176
  end
  object FactOstNez: TRxIBQuery
    Database = DM1.BELMED
    Transaction = DM1.IBT_Read
    CachedUpdates = True
    SQL.Strings = (
      'SELECT  ostatki.Ostatok_begin_Nz,OSTATKI.Ostatok_end_NZ,'
      ' ostatki.Ostatok_begin_s,OSTATKI.Ostatok_end_s,'
      
        'ediz.NEIS,ostatki.NMAT,ostatki.razdel_id,ostatki.ksm_id, OSTATKI' +
        '.KART_ID,RAZDEL.KRAZ,cast(0 as numeric(6,3)) as koef,'
      
        'norm.pr_ov as prsubst,cast(0 as integer) as pr_raspred,norm.plno' +
        'rm,norm.kei_id,ostatki.rash_virab_period,'
      'ostatki.prix_period,'
      'cast(0 as numeric(6,2)) as proc,'
      'cast(0 as numeric(15,6)) as OstFactNz,'
      'cast(0 as numeric(15,6)) as OstFactS,'
      'cast(0 as numeric(15,6)) as ZagFact'
      ' FROM  SELECT_OB_VED (%DAT1,%DAT2,:kodp) ostatki'
      ' Inner JOIN  RAZDEL ON (ostatki.razdel_id =RAZDEL.razdel_id)'
      
        ' left join norm on (ostatki.struk_id=norm.struk_id and ostatki.k' +
        'sm_idpr=norm.kodp and ostatki.ksm_id=norm.ksm_id and razdel.kraz' +
        '=norm.kraz and norm.mes=:mes and norm.god=:god)'
      'left join ediz on (norm.kei_id=ediz.kei_id)'
      ''
      '  ORDER BY ostatki.RAZDEL_ID  '
      ' '
      ''
      ''
      '    '
      ''
      ''
      '')
    UpdateObject = IBUpdateFactOstNez
    OnFilterRecord = FactOstNezFilterRecord
    Macros = <
      item
        DataType = ftString
        Name = 'DAT1'
        ParamType = ptInput
        Value = #39'01.01.2007'#39
      end
      item
        DataType = ftString
        Name = 'DAT2'
        ParamType = ptInput
        Value = #39'31.01.2007'#39
      end>
    Left = 336
    Top = 176
    ParamData = <
      item
        DataType = ftInteger
        Name = 'KODP'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftUnknown
        Name = 'mes'
        ParamType = ptUnknown
      end
      item
        DataType = ftUnknown
        Name = 'god'
        ParamType = ptUnknown
      end>
    object FactOstNezOSTATOK_END_NZ: TFMTBCDField
      FieldName = 'OSTATOK_END_NZ'
      Origin = '"SELECT_OB_VED"."OSTATOK_END_NZ"'
      Precision = 18
      Size = 6
    end
    object FactOstNezNEIS: TIBStringField
      FieldName = 'NEIS'
      Origin = '"SELECT_OB_VED"."NEIS"'
      FixedChar = True
      Size = 10
    end
    object FactOstNezNMAT: TIBStringField
      FieldName = 'NMAT'
      Origin = '"SELECT_OB_VED"."NMAT"'
      Size = 60
    end
    object FactOstNezRAZDEL_ID: TSmallintField
      FieldName = 'RAZDEL_ID'
      Origin = '"SELECT_OB_VED"."RAZDEL_ID"'
    end
    object FactOstNezKART_ID: TIntegerField
      FieldName = 'KART_ID'
      Origin = '"SELECT_OB_VED"."KART_ID"'
    end
    object FactOstNezKSM_ID: TIntegerField
      FieldName = 'KSM_ID'
      Origin = '"SELECT_OB_VED"."KSM_ID"'
    end
    object FactOstNezOSTATOK_BEGIN_NZ: TFMTBCDField
      FieldName = 'OSTATOK_BEGIN_NZ'
      Origin = '"SELECT_OB_VED"."OSTATOK_BEGIN_NZ"'
      Precision = 18
      Size = 6
    end
    object FactOstNezKRAZ: TSmallintField
      FieldName = 'KRAZ'
      Origin = '"RAZDEL"."KRAZ"'
    end
    object FactOstNezOSTATOK_END_S: TFMTBCDField
      FieldName = 'OSTATOK_END_S'
      Origin = '"SELECT_OB_VED"."OSTATOK_END_S"'
      Precision = 18
      Size = 6
    end
    object FactOstNezOSTATOK_BEGIN_S: TFMTBCDField
      FieldName = 'OSTATOK_BEGIN_S'
      Origin = '"SELECT_OB_VED"."OSTATOK_BEGIN_S"'
      Precision = 18
      Size = 6
    end
    object FactOstNezKOEF: TIBBCDField
      FieldName = 'KOEF'
      ProviderFlags = []
      Precision = 9
      Size = 3
    end
    object FactOstNezPRSUBST: TSmallintField
      FieldName = 'PRSUBST'
      Origin = '"NORM"."PR_OV"'
    end
    object FactOstNezPR_RASPRED: TIntegerField
      FieldName = 'PR_RASPRED'
      ProviderFlags = []
    end
    object FactOstNezPLNORM: TFMTBCDField
      FieldName = 'PLNORM'
      Origin = '"NORM"."PLNORM"'
      Precision = 18
      Size = 6
    end
    object FactOstNezPROC: TIBBCDField
      FieldName = 'PROC'
      ProviderFlags = []
      Precision = 9
      Size = 2
    end
    object FactOstNezOSTFACTNZ: TFMTBCDField
      FieldName = 'OSTFACTNZ'
      ProviderFlags = []
      Precision = 18
      Size = 6
    end
    object FactOstNezZAGFACT: TFMTBCDField
      FieldName = 'ZAGFACT'
      ProviderFlags = []
      Precision = 18
      Size = 6
    end
    object FactOstNezKEI_ID: TSmallintField
      FieldName = 'KEI_ID'
      Origin = '"NORM"."KEI_ID"'
    end
    object FactOstNezOSTFACTS: TFMTBCDField
      FieldName = 'OSTFACTS'
      ProviderFlags = []
      Precision = 18
      Size = 6
    end
    object FactOstNezRASH_VIRAB_PERIOD: TFMTBCDField
      FieldName = 'RASH_VIRAB_PERIOD'
      Origin = '"SELECT_OB_VED"."RASH_VIRAB_PERIOD"'
      Precision = 18
      Size = 6
    end
    object FactOstNezPRIX_PERIOD: TFMTBCDField
      FieldName = 'PRIX_PERIOD'
      Origin = '"SELECT_OB_VED"."PRIX_PERIOD"'
      Precision = 18
      Size = 6
    end
  end
  object DSFactOstNez: TDataSource
    DataSet = FactOstNez
    Left = 416
    Top = 176
  end
  object IBUpdateFactOstNez: TIBUpdateSQLW
    RefreshSQL.Strings = (
      'Select '
      '  DOC_ID,'
      '  STROKA_ID,'
      '  KSM_ID,'
      '  CENA,'
      '  KOL_PRIH,'
      '  CENA_VP,'
      '  NDS,'
      '  SUM_NDS,'
      '  SUM_NDS_VP,'
      '  KOL_RASH,'
      '  KART_ID,'
      '  ORG_ID_BRAK,'
      '  OSNOV_BRAK_ID,'
      '  USER_NAME,'
      '  DATE_TIME_UPDATE,'
      '  PARENT,'
      '  RAZDEL_ID,'
      '  SKIDKA,'
      '  KEI_ID,'
      '  SUMMA,'
      '  SUMMA_VP,'
      '  SUM_SKID,'
      '  SUM_SKID_VP,'
      '  SUMMA_S_NDS,'
      '  SUMMA_S_NDS_VP,'
      '  TAG,'
      '  KOL_PRIH_EDIZ,'
      '  KOL_RASH_EDIZ'
      'from KART '
      'where'
      '  STROKA_ID = :STROKA_ID')
    ModifySQL.Strings = (
      'select *'
      'from kart'
      '  where'
      '  STROKA_ID = :OLD_STROKA_ID')
    InsertSQL.Strings = (
      'select *'
      'from kart'
      '  where'
      '  STROKA_ID = :OLD_STROKA_ID')
    AutoCommit = False
    UpdateTransaction = DM1.IBT_Write
    Left = 376
    Top = 176
  end
  object SyrieZag: TRxIBQuery
    Database = DM1.BELMED
    Transaction = DM1.IBT_Read
    CachedUpdates = True
    SQL.Strings = (
      
        'SELECT kart.kart_id,KART.Ksm_Id,matrop.NMAT, matrop.XARKT,KART.K' +
        'EI_ID,KART.KOL_RASH_ediz rasvir,spprod.nmat nmat_prod, EDIZ.NEIS' +
        ',OSTATKI.KSM_IDPR KODP,'
      
        ' (z(KART. KOL_RASH)/Z(KARTV.KOL_PRIH))  KOEF_Zag,   SPPROD.KOD_P' +
        'ROD,razdel.kraz,region.nam,'
      'ostatki.razdel_id,'
      'cast(0 as numeric(15,6)) as prix,'
      'cast(0 as numeric(15,6)) as kol_rash_ediz,'
      'ostatki.ostatok_begin_nz,'
      'ostatki.ostatok_begin_s,'
      'ostatki.ostatok_end_nz,'
      'ostatki.ostatok_end_s'
      ' FROM KART'
      '  INNER JOIN MATROP ON (KART.KSM_ID = MATROP.KSM_ID)'
      '  INNER JOIN EDIZ ON (KART.KEI_ID = EDIZ.KEI_ID)'
      '  INNER JOIN DOCUMENT ON (KART.DOC_ID = DOCUMENT.DOC_ID)'
      '  INNER JOIN RAZDEL ON (KART.razdel_ID = RAZDEL.razdel_ID)'
      
        '  INNER JOIN SELECT_OB_VED (%DAT1,%DAT2,:kodp) OSTATKI ON (KART.' +
        'KART_ID = OSTATKI.KART_ID)'
      '  left JOIN KARTV ON (document.Klient_ID = KARTV.KSM_ID)'
      '  INNER JOIN SPPROD ON (DOCUMENT.KLIENT_ID = SPPROD.KSM_ID)'
      '  left JOIN region ON (spprod.reg = region.reg)'
      ' WHERE DOCUMENT.STRUK_ID=:strk'
      ' AND DOCUMENT.TIP_OP_ID=35 '
      ' AND DOCUMENT.NDOK containing  %DOC'
      'AND DOCUMENT.NDOK not containing  %DOCY'
      ' AND  Document.Date_dok between :dat1 and :dat2'
      
        'and kartv.doc_id in (select document.doc_id from document where ' +
        '(Document.Date_dok between :dat1 and :dat2)'
      '   and document.tip_op_id=36 and DOCUMENT.STRUK_ID=:strk)'
      ' order by %Sort'
      ' '
      ''
      ''
      '    '
      ''
      ''
      '')
    UpdateObject = IBUpdateSyrieZag
    Macros = <
      item
        DataType = ftString
        Name = 'DAT1'
        ParamType = ptInput
        Value = #39'01.07.2007'#39
      end
      item
        DataType = ftString
        Name = 'DAT2'
        ParamType = ptInput
        Value = #39'31.07.2007'#39
      end
      item
        DataType = ftString
        Name = 'DOC'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'DOCY'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'Sort'
        ParamType = ptInput
        Value = 'spprod.kod_prod'
      end>
    Left = 320
    Top = 504
    ParamData = <
      item
        DataType = ftInteger
        Name = 'kodp'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftInteger
        Name = 'strk'
        ParamType = ptInput
        Value = '1'
      end
      item
        DataType = ftDateTime
        Name = 'dat1'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = 'dat2'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = 'dat1'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = 'dat2'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'strk'
        ParamType = ptInput
      end>
    object SyrieZagKART_ID: TSmallintField
      FieldName = 'KART_ID'
      Origin = '"KART"."KART_ID"'
      Required = True
    end
    object SyrieZagKSM_ID: TIntegerField
      FieldName = 'KSM_ID'
      Origin = '"KART"."KSM_ID"'
      Required = True
    end
    object SyrieZagNMAT: TIBStringField
      FieldName = 'NMAT'
      Origin = '"MATROP"."NMAT"'
      Size = 60
    end
    object SyrieZagXARKT: TIBStringField
      FieldName = 'XARKT'
      Origin = '"MATROP"."XARKT"'
      Size = 30
    end
    object SyrieZagKEI_ID: TSmallintField
      FieldName = 'KEI_ID'
      Origin = '"KART"."KEI_ID"'
    end
    object SyrieZagRASVIR: TFloatField
      FieldName = 'RASVIR'
      Origin = '"KART"."KOL_RASH_EDIZ"'
    end
    object SyrieZagNMAT_PROD: TIBStringField
      FieldName = 'NMAT_PROD'
      Origin = '"SPPROD"."NMAT"'
      Size = 60
    end
    object SyrieZagNEIS: TIBStringField
      FieldName = 'NEIS'
      Origin = '"EDIZ"."NEIS"'
      FixedChar = True
      Size = 10
    end
    object SyrieZagKODP: TIntegerField
      FieldName = 'KODP'
      Origin = '"OSTATKI"."KSM_IDPR"'
    end
    object SyrieZagKOEF_ZAG: TFloatField
      FieldName = 'KOEF_ZAG'
      ProviderFlags = []
    end
    object SyrieZagKOD_PROD: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'KOD_PROD'
      Origin = '"SPPROD"."KOD_PROD"'
      ProviderFlags = []
      ReadOnly = True
      FixedChar = True
      Size = 18
    end
    object SyrieZagKRAZ: TSmallintField
      FieldName = 'KRAZ'
      Origin = '"RAZDEL"."KRAZ"'
    end
    object SyrieZagNAM: TIBStringField
      FieldName = 'NAM'
      Origin = '"REGION"."NAM"'
      FixedChar = True
    end
    object SyrieZagRAZDEL_ID: TSmallintField
      FieldName = 'RAZDEL_ID'
      Origin = '"OSTATKI"."RAZDEL_ID"'
    end
    object SyrieZagPRIX: TFMTBCDField
      FieldName = 'PRIX'
      ProviderFlags = []
      Precision = 18
      Size = 6
    end
    object SyrieZagKOL_RASH_EDIZ: TFMTBCDField
      FieldName = 'KOL_RASH_EDIZ'
      ProviderFlags = []
      Precision = 18
      Size = 6
    end
    object SyrieZagOSTATOK_BEGIN_NZ: TFMTBCDField
      FieldName = 'OSTATOK_BEGIN_NZ'
      Origin = '"SELECT_OB_VED"."OSTATOK_BEGIN_NZ"'
      Precision = 18
      Size = 6
    end
    object SyrieZagOSTATOK_BEGIN_S: TFMTBCDField
      FieldName = 'OSTATOK_BEGIN_S'
      Origin = '"SELECT_OB_VED"."OSTATOK_BEGIN_S"'
      Precision = 18
      Size = 6
    end
    object SyrieZagOSTATOK_END_NZ: TFMTBCDField
      FieldName = 'OSTATOK_END_NZ'
      Origin = '"SELECT_OB_VED"."OSTATOK_END_NZ"'
      Precision = 18
      Size = 6
    end
    object SyrieZagOSTATOK_END_S: TFMTBCDField
      FieldName = 'OSTATOK_END_S'
      Origin = '"SELECT_OB_VED"."OSTATOK_END_S"'
      Precision = 18
      Size = 6
    end
  end
  object IBUpdateSyrieZag: TIBUpdateSQLW
    RefreshSQL.Strings = (
      'Select '
      '  DOC_ID,'
      '  STROKA_ID,'
      '  KSM_ID,'
      '  CENA,'
      '  KOL_PRIH,'
      '  CENA_VP,'
      '  NDS,'
      '  SUM_NDS,'
      '  SUM_NDS_VP,'
      '  KOL_RASH,'
      '  KART_ID,'
      '  ORG_ID_BRAK,'
      '  OSNOV_BRAK_ID,'
      '  USER_NAME,'
      '  DATE_TIME_UPDATE,'
      '  PARENT,'
      '  RAZDEL_ID,'
      '  SKIDKA,'
      '  KEI_ID,'
      '  SUMMA,'
      '  SUMMA_VP,'
      '  SUM_SKID,'
      '  SUM_SKID_VP,'
      '  SUMMA_S_NDS,'
      '  SUMMA_S_NDS_VP,'
      '  TAG,'
      '  KOL_PRIH_EDIZ,'
      '  KOL_RASH_EDIZ'
      'from KART '
      'where'
      '  STROKA_ID = :STROKA_ID')
    ModifySQL.Strings = (
      'select *'
      'from kart'
      '  where'
      '  STROKA_ID = :OLD_STROKA_ID')
    InsertSQL.Strings = (
      'select *'
      'from kart'
      '  where'
      '  STROKA_ID = :OLD_STROKA_ID')
    AutoCommit = False
    UpdateTransaction = DM1.IBT_Write
    Left = 376
    Top = 504
  end
  object FactVipuskZag: TRxIBQuery
    Database = DM1.BELMED
    Transaction = DM1.IBT_Read
    CachedUpdates = True
    SQL.Strings = (
      
        'SELECT KARTV.KOL_PRIH,SPPROD.NMAT, SPPROD.XARKT,SPPROD.KOD_PROD,' +
        'KARTV.KSM_ID,region.nam region,cast(0 as numeric(15,6)) as VipNz'
      ' FROM KARTV'
      ' INNER JOIN SPPROD ON (KARTV.KSM_ID = SPPROD.KSM_ID)'
      ' INNER JOIN REGION ON (SPPROD.REG = REGION.REG)'
      ' INNER JOIN DOCUMENT ON (KARTV.DOC_ID = DOCUMENT.DOC_ID)'
      ' WHERE DOCUMENT.TIP_OP_ID=36'
      ' AND DOCUMENT.STRUK_ID=:strk'
      ' AND Document.Date_op between :dat1 and :dat2'
      ' AND KARTV.KSM_ID in (SELECT DOCUMENT.KLIENT_ID '
      '  FROM DOCUMENT '
      '  WHERE DOCUMENT.NDOK containing  %DOC'
      '    and DOCUMENT.NDOK not containing %DOCY'
      '    AND Document.Date_dok between :dat1 and :dat2)'
      ' ORDER BY %SORT'
      ''
      ''
      '    '
      ''
      ''
      '')
    UpdateObject = IBUpdateFactVipuskZag
    Macros = <
      item
        DataType = ftString
        Name = 'DOC'
        ParamType = ptInput
        Value = #39#39
      end
      item
        DataType = ftString
        Name = 'DOCY'
        ParamType = ptInput
        Value = #39#39
      end
      item
        DataType = ftString
        Name = 'SORT'
        ParamType = ptInput
        Value = 'SPPROD.KOD_PROD'
      end>
    Left = 296
    Top = 384
    ParamData = <
      item
        DataType = ftInteger
        Name = 'strk'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = 'dat1'
        ParamType = ptInput
        Value = '01.01.07'
      end
      item
        DataType = ftDateTime
        Name = 'dat2'
        ParamType = ptInput
        Value = '01.01.07'
      end
      item
        DataType = ftDateTime
        Name = 'dat1'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = 'dat2'
        ParamType = ptInput
      end>
    object FactVipuskZagKOL_PRIH: TFMTBCDField
      FieldName = 'KOL_PRIH'
      Origin = '"KARTV"."KOL_PRIH"'
      Precision = 18
      Size = 6
    end
    object FactVipuskZagNMAT: TIBStringField
      FieldName = 'NMAT'
      Origin = '"SPPROD"."NMAT"'
      Size = 60
    end
    object FactVipuskZagXARKT: TIBStringField
      FieldName = 'XARKT'
      Origin = '"SPPROD"."XARKT"'
      Size = 30
    end
    object FactVipuskZagKOD_PROD: TIBStringField
      FieldKind = fkInternalCalc
      FieldName = 'KOD_PROD'
      Origin = '"SPPROD"."KOD_PROD"'
      ProviderFlags = []
      ReadOnly = True
      FixedChar = True
      Size = 18
    end
    object FactVipuskZagKSM_ID: TIntegerField
      FieldName = 'KSM_ID'
      Origin = '"KARTV"."KSM_ID"'
      Required = True
    end
    object FactVipuskZagREGION: TIBStringField
      FieldName = 'REGION'
      Origin = '"REGION"."NAM"'
      FixedChar = True
    end
    object FactVipuskZagVIPNZ: TFMTBCDField
      FieldName = 'VIPNZ'
      ProviderFlags = []
      Precision = 18
      Size = 6
    end
  end
  object IBUpdateFactVipuskZag: TIBUpdateSQLW
    RefreshSQL.Strings = (
      'Select '
      '  DOC_ID,'
      '  STROKA_ID,'
      '  KSM_ID,'
      '  CENA,'
      '  KOL_PRIH,'
      '  CENA_VP,'
      '  NDS,'
      '  SUM_NDS,'
      '  SUM_NDS_VP,'
      '  KOL_RASH,'
      '  KART_ID,'
      '  ORG_ID_BRAK,'
      '  OSNOV_BRAK_ID,'
      '  USER_NAME,'
      '  DATE_TIME_UPDATE,'
      '  PARENT,'
      '  RAZDEL_ID,'
      '  SKIDKA,'
      '  KEI_ID,'
      '  SUMMA,'
      '  SUMMA_VP,'
      '  SUM_SKID,'
      '  SUM_SKID_VP,'
      '  SUMMA_S_NDS,'
      '  SUMMA_S_NDS_VP,'
      '  TAG,'
      '  KOL_PRIH_EDIZ,'
      '  KOL_RASH_EDIZ'
      'from KART '
      'where'
      '  STROKA_ID = :STROKA_ID')
    ModifySQL.Strings = (
      'select *'
      'from kart'
      '  where'
      '  STROKA_ID = :OLD_STROKA_ID')
    InsertSQL.Strings = (
      'select *'
      'from kart'
      '  where'
      '  STROKA_ID = :OLD_STROKA_ID')
    DeleteSQL.Strings = (
      'select *'
      'from kart'
      '  where'
      '  STROKA_ID = :OLD_STROKA_ID')
    AutoCommit = False
    UpdateTransaction = DM1.IBT_Write
    Left = 352
    Top = 384
  end
  object FormStorage1: TFormStorage
    StoredValues = <>
    Left = 472
    Top = 200
  end
  object DataSource1: TDataSource
    DataSet = DM1.Kart
    Left = 112
    Top = 384
  end
end
