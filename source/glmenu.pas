...
procedure TFGlMenu.N53Click(Sender: TObject);
begin
  if (RXLabel1.Caption = '') then
  begin
    MessageDlg('Задайте подразделение !', mtWarning, [mbOK], 0);
  ...
  if (FTechJourn = nil) then                       //
    FTechJourn := TFTechJourn.Create(Application);  //
  FTechJourn.Parent := Main_Panel;             //
  if (ActiveForm <> nil) then
    ActiveForm.Close;
  ActiveForm := FTechJourn;                  //
