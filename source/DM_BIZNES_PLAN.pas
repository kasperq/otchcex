...
uses
  ...
  frxDMPExport, frxDBSet, frxIBXComponents;
//  , DBTables;
 ...
 type
  TDM1 = class(TDataModule)
    ...
    OstatkiOT_DOKUM: TFMTBCDField;
    OstatkiONM_DOKUM: TFloatField;
    ...
    function Koef_per(kei_spr: integer;kei: integer;ksm: integer): variant;
...
procedure TDM1.DobPrixPrep;
 ...
 begin
  ...
  DM1.Ostatki.MacroByName('usl').AsString:=' and (ost.ksm_idpr is null or OST.KSM_IDPR=0) AND OST.KSM_ID='+INTTOSTR(s_ksm);
  dm1.Ostatki.Open;
  if dm1.Ostatki.Eof then st_kart:=0 else st_kart:=dm1.OstatkiKart_id.AsInteger;
  if (not dm1.Ostatki.Eof) and ((dm1.OstatkiOt_s.AsFloat>v_raspred)or(((dm1.OstatkiOt_s.AsFloat-v_raspred)>-0.00000001))and((dm1.OstatkiOt_s.AsFloat-v_raspred)<0.00000001))  then
   pr_zanes:=True
  else
  begin
//   v_raspred_dob:=0;
   if pr_button9 then
    if MessageDlg('Нет сырья '+inttostr(s_ksm)+' '+DM1.OstatkiNmat.asstring+' для загрузки! Вводить загрузку по этому сырью?', mtConfirmation, [mbYes, mbNo], 0)=mrYes   then
     pr_zanes:=true
    else
     pr_zanes:=False;
   end;
//                 запись в Kart прихода сырья  на препарат
   if pr_zanes then
   begin
    dat:=strtodate(s_dat1);
    if dm1.Document.Active then
    begin
     if dm1.Document.Locate('struk_id;tip_op_id;klient_id;date_op',VarArrayOf([vStruk_id,30,vKlient_id,dat]),[]) then
     begin
      ...
      dm1.KartKol_rash.AsVariant:=0;
      dm1.KartKol_prih.AsVariant:=0;
      ...
      DM1.Kart.FieldByName('KART_Id').AsInteger := st_Kart;
     ...
     begin
      ...
      BEGIN
       vTip_Op_Id:=30;
       vTip_Doc_Id:=37;
       vNDoc:='Пд-'+inttostr(S_Kodp)+'-'+inttostr(mes)+'.'+inttostr(god);
       vKlient_Id:=S_KODP;
       ...
       dm1.KartKol_RASH_ediz.AsVariant:=v_raspred_dob;
  ...
  else v_raspred_dob:=0;
{   if pr_button9 then
    if MessageDlg('Нет сырья '+inttodtr(s_ksm)+''''+DM1.Ostatki.nmat.asstring+''''+' для загрузки!Продолжить?', mtConfirmation, [mbYes, mbNo], 0)=mrYes then
    MessageDlg('Нет сырья для загрузки!', mtWarning, [mbOK], 0);
  end;}
  ...
  begin
   ...
   begin
    ...
    begin
     ...
     if FVibPrep.RadioGroup4.ItemIndex=1 then
     begin
{      If not dm1.IBT_Read.Active then dm1.IBT_Read.StartTransaction;
      IF DM1.Kart.Active THEN DM1.Kart.Close;
      DM1.Kart.MacroByName('USL').AsString:=' WHERE DOCUMENT.TIP_OP_ID IN (30,33,35,34,37) AND DOCUMENT.TIP_DOK_ID=37 '
      +' AND Document.Date_op between '+''''+s_dat1+'''' +' and '+''''+s_dat2+''''
      ...
      begin
       S_KODP:=DM1.KartKLIENT_ID.AsInteger;
       V_RAZDEL:=DM1.KartRAZDEL_ID.AsInteger;
       if FVibPrep.FactVipusk.Locate('KSM_ID;VIB',VarArrayOf([S_KODP,TRUE]),[]) THEN
        if FVibPrep.RAZDEL1.Locate('RAZDEL_ID;VIB',VarArrayOf([V_RAZDEL,TRUE]),[]) THEN
         DM1.Kart.Delete
        ELSE DM1.Kart.Next
       ELSE DM1.Kart.Next;
      end;
     ...
     begin
      ...
      begin
       ...
       DM1.IBQuery1.SQL.Add('SELECT kartv.kol_prih,KARTv.KSM_ID,razdel.razdel_id,NORM.PLNORM,norm.kei_id ,kartv.doc_id,cast(norm.plnorm*kartv.kol_prih as numeric(16,7))  as raspred ');
...
{     if FVibPrep.RadioGroup4.ItemIndex=1 then
     begin
      If not dm1.IBT_Read.Active then dm1.IBT_Read.StartTransaction;
      IF DM1.Kart.Active THEN DM1.Kart.Close;
      DM1.Kart.MacroByName('USL').AsString:=' WHERE DOCUMENT.TIP_OP_ID IN (37,33,34) AND DOCUMENT.TIP_DOK_ID=37 '
      +' AND Document.Date_op between '+''''+s_dat1+'''' +' and '+''''+s_dat2+''''
      +' AND DOcUMENT.STRUK_ID='+INTTOSTR(VsTRUK_ID)
      +' AND KART.ksm_id='+INTTOSTR(s_ksm)+'and kart.parent is null';
      DM1.Kart.Open;
     ...
     IBQuery1.SQL.Clear;
     DM1.IBQuery1.SQL.Add('SELECT distinct kartv.kol_prih,KARTv.KSM_ID,razdel.razdel_id,NORM.PLNORM,norm.kei_id ,kartv.doc_id,cast(norm.plnorm*kartv.kol_prih as numeric(16,7)) as raspred ');
     DM1.IBQuery1.SQL.Add(' FROM NORM ');
     DM1.IBQuery1.SQL.Add(' INNER JOIN KARTv ON (NORM.KODP = KARTv.KSM_ID) ');
     DM1.IBQuery1.SQL.Add(' INNER JOIN razdel ON (NORM.kraz = razdel.Kraz) ');
     DM1.IBQuery1.SQL.Add(' INNER JOIN document ON (kartv.doc_id = document.doc_id)');
...
//     begin
      {If not dm1.IBT_Read.Active then dm1.IBT_Read.StartTransaction;
      IF DM1.Kart.Active THEN DM1.Kart.Close;
      DM1.Kart.MacroByName('USL').AsString:=' WHERE DOCUMENT.TIP_OP_ID IN (37,35) AND DOCUMENT.TIP_DOK_ID=37 '
      +' AND Document.Date_op between '+''''+s_dat1+'''' +' and '+''''+s_dat2+''''
      +' AND DOcUMENT.STRUK_ID='+INTTOSTR(VsTRUK_ID)
      +' AND KART.ksm_id='+INTTOSTR(s_ksm);
      DM1.Kart.Open;
...
begin
 if st_kart=0 then vKart_Id:=0 else vkart_id:=st_kart;
 s_seria:='';
...
begin
 ...
 DM1.Kart.FieldByName('Klient_Id').AsInteger := vKlient_Id;
 if DM1.Kart.FieldByName('kol_prih').AsVariant=null then DM1.Kart.FieldByName('kol_prih').AsVariant:=0;
 if DM1.Kart.FieldByName('kol_rash').AsVariant=null then DM1.Kart.FieldByName('kol_rash').AsVariant:=0;
 if DM1.Kart.FieldByName('kol_prih_ediz').AsVariant=null then DM1.Kart.FieldByName('kol_prih_ediz').AsFloat:=0;
 if DM1.Kart.FieldByName('kol_rash_ediz').AsVariant=null then DM1.Kart.FieldByName('kol_rash_ediz').AsFloat:=0;
end;
...
//  DM1.Document.FieldByName('Zadacha_Id').AsString := vZadacha_Id;
  ...
  if copy(vNdoc,1,5)='Старт' then vNdoc:=vndoc+inttostr(vDocument_id);
...
begin
  ...
  if DM1.SeriaKol_Seria.AsVariant='' then DM1.SeriaKol_Seria.AsFloat:=0;
end;
...
begin
    ...
    DM1.Ostatki.FieldByName('Mes').AsInteger :=Mes;
    DM1.Ostatki.FieldByName('God').AsInteger :=God;
 end;

 function TDM1.MesName(Mes: integer): string;
begin
  case Mes of
    1: Result:='Января';
    2: Result:='Февраля';
...
function TDM1.Koef_per(kei_spr: integer;kei: integer;ksm: integer): variant;
  var
  vKoef: variant;
begin
 if kei_spr<>kei then
...
// Коэффициент перевода  из одной единицы измерения в другую
  ...
  IntToStr(Kei_Spr)+
  ' AND KOEFPR.KEI_ID = '+ IntToStr(Kei)+
  ' AND KOEFPR.KSM_ID = 0 AND KOEFPR.KOEF<>0 ',BELMED, IBT_Read);
  if vKoef = NULL then
  begin
   vKoef := SelectToVarIB('SELECT KOEFPR.KOEF FROM KOEFPR WHERE KOEFPR.KEIR = '+
   IntToStr(Kei_Spr)+
   ' AND KOEFPR.KEI_ID = '+ IntToStr(Kei)+
   ' AND KOEFPR.KSM_ID ='+IntToStr(Ksm)+
   ' AND KOEFPR.KOEF<>0 ',BELMED, IBT_Read);
   if vKoef = NULL then
    vKoef := 1;
  end;
  Result:=vKoef;
...
procedure TDM1.frReport2GetValue(const ParName: String;
  var ParValue: Variant);
...
begin
if mes=1 then  nammes:= 'январь';
if mes=2 then  nammes:= 'февраль';
if mes=3 then  nammes:= 'март';
if mes=4 then nammes := 'апрель';
if mes=5 then nammes := 'май';
if mes=6 then nammes := 'июнь';
if mes=7 then  nammes:= 'июль';
if mes=8 then nammes := 'август';
if mes=9 then  nammes:= 'сентябрь';
if mes=10 then nammes := 'октябрь';
if mes=11 then nammes:= 'ноябрь';
if mes=12 then  nammes:= 'декабрь';
if VarName='nammes' then
begin
 Value:=nammes;
end;
if VarName='stname' then Value:=FGlMenu.RxLabel1.Caption;
if VarName='lekkod' then Value:=FOtchet.Edit1.Text;
if VarName='gostp' then Value:=FOtchet.Label22.Caption;
if VarName='xarkt' then Value:=FOtchet.Label41.Caption;
if VarName='namorg' then Value:=FOtchet.Label29.Caption;
if VarName='nam_prep' then Value:=FOtchet.Label19.Caption;
if VarName='neiz_prep' then Value:=FOtchet.Label21.Caption;
if VarName='god' then Value:=god;
if VarName='sdano_m' then Value:=FOtchet.Label1.Caption;
if VarName='sdano_g' then Value:=FOtchet.Label14.Caption;
if VarName='reg' then
...
begin
 ...
 if FSprFormul.Ceh_Normz.Active=true then FSprFormul.CEH_NormZ.Close;
 FSprFormul.CEH_NormZ.MacroByName('SORT').AsString:='';
 FSprFormul.CEH_NormZ.MacroByName('USL').AsString:=' Where CEH_NORMZ.KSM_ID_PR='+INTTOSTR(dm1.PechOtchetKodp.asInteger)+' and CEH_NORMZ.KSM_ID_MAt='+INTTOSTR(dm1.PechOtchetKsm_id.AsInteger);
 FSprFormul.CEH_NormZ.Open;
 if not FSprFormul.CEH_NORMZ.eof then
...
begin
 if dm1.PechOtchetKodp.AsVariant=star_kod then Value:=1
 else
  Value:=2;
...
begin
 ...
 If (DM1.Kart.FieldByName('Kei_id').AsInteger = 0) and ((DM1.Kart.FieldByName('Kol_prih_ediz').AsFloat<>0) or (DM1.Kart.FieldByName('Kol_rash_ediz').AsFloat<>0)) then
 begin
 ...
 try
  ...
  begin
   ...
   DM1.Ostatki.ParamByName('struk_ID').AsInteger:=vstruk_id;
   if v_Razdel=0 then
    DM1.Ostatki.MacroByName('usl').AsString:=' and (ost.razdel_id is null OR ost.razdel_id='+inttostr(v_Razdel)+')'
